...
function parseNode(node, params) {
  ...
  if (node.type === 'rule') {
    ...
    node.selector = node.selector.replace(
      /(?::(local|global)\s*\(([^)]+)\))|,|:(local|global)|\.([^\s.,#:()[\]]+)|([\S]+::[\S]+)/g,
      (str, scopeName, content, scopeNameChanger, className, moduleRef) => {
        if (moduleRef) {
          const [moduleId, moduleClassName] = moduleRef.split('::')
...
function defaultResolvePath(filepath, dirpath) {
...
module.exports = postcss.plugin('cssm', (opts = {}) => {
  ...
  return (css, result) => {
    ...
    const resolvePath = filepath => {
      ...
      return (opts.resolvePath || defaultResolvePath)(filepathWithExt, dirpath, defaultResolvePath)
    }
